...
async def reformat_laporan(file: UploadFile):
    ...
    try:
        if not file.filename.endswith(".pdf"):
            return {"status": "error", "message": "File harus dalam format pdf"}
        ...
        return {"status": "error", "message": "Tidak ada tabel dalam laporan"}
...
async def reformat_referensi(file: UploadFile):
    ...
    try:
        if not file.filename.endswith(".xlsx"):
            return {"status": "error", "message": "File harus dalam format xlsx"}
